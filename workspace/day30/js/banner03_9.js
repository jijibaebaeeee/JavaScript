...
//슬라이드 바스 너비, 인덱스번호, 총 슬라이드 이미지 수
let slideWidth = 800; // CSS에서 800으로정했으니까
let currentIdx = 0;
let slideCnt = $slideImgs.length // $slideImgs는 jquery여도 .length 붙으면 숫자가 된다. $필요 없음
...
//슬라이드 박스에 마우스 hover시 자동 슬라이드 중지
$slideBox.parent().on("mouseenter", stopSlide);

//슬라이드 박스에서 마우스가 떨어지면 자동으로 슬라이드 시작
