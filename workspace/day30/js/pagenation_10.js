...
function createPost(content){
  ...
  post.classList.add("post"); // div태그에 post 클래스 추가
...
function showPage(page){
  contentContainer.innerHTML = ""; // 페이지가 바뀌면 현재 페이지는 초기화가 되어야하므로
  const startIdx = (page - 1) * pageSize; // 인덱스는 1씩 작으니까 0, 5, 10, 15가 맞음
...
for(let i = 1; i <= totalPages; i++){
  ...
  if(i === 1){
    li.classList.add('active'); // 현재 보고있다는 페이지를 더욱 꾸며주기 위해 클래스 추가
  }

  li.addEventListener("click", onPageClick); //페이지 번호 클릭 이벤트 핸들러 추가
  pagenation.appendChild(li); // for문 맨 처음 li 추가하는 것
                              // 페이지 번호를 <ul>자식 요소로 <li>요소 추가
}

pagenationContainer.appendChild(pagenation);
showPage(1); //무조건 1페이지부터 시작하게 할 수 있다.
